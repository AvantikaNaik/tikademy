...
def match():
    if request.method == "POST":
        ...
        schedule_info = db.execute("SELECT match.user_id FROM schedule match JOIN schedule main WHERE main.user_id=:main_id AND main.user_id != match.user_id AND match.role=:match_role AND (main.v_8M=match.v_8M OR main.v_8T=match.v_8T OR main.v_8W=match.v_8W OR main.v_8R=match.v_8R OR main.v_8F=match.v_8F OR main.v_8S=match.v_8S OR main.v_8U=match.v_8U OR        main.v_10M=match.v_10M OR main.v_10T=match.v_10T OR main.v_10W=match.v_10W OR main.v_10R=match.v_10R OR main.v_10F=match.v_10F OR main.v_10S=match.v_10S OR main.v_10U=match.v_10U OR         main.v_10M=match.v_10M OR main.v_10T=match.v_10T OR main.v_10W=match.v_10W OR main.v_10R=match.v_10R OR main.v_10F=match.v_10F OR main.v_10S=match.v_10S OR main.v_10U=match.v_10U )", main_id=session["user_id"], match_role=match_role )

        schedule_match_usernames = []

        for match in schedule_info:
            schedule_id = match["user_id"]
            schedule_match_usernames.append(schedule_id)


        overlap = list(set(schedule_match_usernames) & set(grade_match_usernames) & set(subject_match_usernames) )


        #Housekeeping
        matchdata = []
        if len(overlap) >= 1:
            for matchid in overlap:
                db.execute("UPDATE users SET active=:active WHERE id=:id", active="no", id=session["user_id"])
                db.execute("UPDATE users SET active=:active WHERE id=:id", active="no", id=matchid)
                data = db.execute("SELECT firstname, email FROM users WHERE id=:id", id=matchid)
                name = data[0]["firstname"]
                email = data[0]["email"]
                matchdata.append((name, email))

        match_count = len(matchdata)

        return render_template("matched.html", match_count = match_count, matchdata = matchdata)
    else:
...
def matched():
    if request.method == "GET":
        render_template("matched.html", match_count = match_count, matchdata = matchdata)
    else:
